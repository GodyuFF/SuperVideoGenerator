...
# 快捷启动
dev.bat                          # Windows: 一键启动 API + 前端（或 --api / --web 单独启动）
dev-desktop.bat                  # Windows: API + 前端 + Electron 桌面壳
dev.sh                           # macOS/Linux: 一键启动 API + 前端（或 --api / --web 单独启动）
start_api.bat                    # Windows 仅启动后端
start_web.bat                    # Windows 仅启动前端
```
