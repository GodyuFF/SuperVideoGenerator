...
from core.prompt.loader import load_required
...
def test_react_system_prompt_loaded_from_file():
    ...
    assert text == load_required("rules/react_json.md")


def test_action_json_system_base_loaded():
    text = get_action_json_system_base()
    assert "observation" in text
    assert "JSON" in text


def test_agent_actions_prompt_loaded():
    from core.prompt.registry import get_agent_actions_prompt

    text = get_agent_actions_prompt("script_agent")
    assert "parse_brief" in text
    assert "asset_id" in text


def test_fixed_role_path():
    from core.prompt.registry import get_agent_role_prompt
...
def test_extract_role_summary():
    from core.prompt.registry import extract_role_summary
