...
def test_bind_style_on_first_call():
    ...
    assert mode == VideoStyleMode.AI_VIDEO.value
    assert script.style_mode == VideoStyleMode.AI_VIDEO.value
...
def test_bind_style_uses_project_default_when_not_requested():
    """未指定风格时使用项目默认。"""
    project = Project(title="p")
    project.config.style.mode = VideoStyleMode.STORYBOOK.value
    script = Script(project_id=project.id, title="s")

    mode = bind_script_style(script, project, None)
    assert mode == VideoStyleMode.STORYBOOK.value
    assert script.style_locked is True
...
def test_cannot_change_locked_style():
    ...
    project = Project(title="p")
    script = Script(project_id=project.id, title="s")
    bind_script_style(script, project, VideoStyleMode.STORYBOOK)
