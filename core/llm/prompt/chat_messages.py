...
3. **硬性独占**：`delegate_agent` / `finish` / `ask_user_question` / `update_plan` / `replan` 本轮 tool_calls **只能各占一轮且不可与其他 function 并列**；禁止与 `tool_*` 同轮。若 observation 含「不可与其他 tool 同轮调用」，下一轮立即改为只调那一个独占 action。
...
8. **计划跟踪**：进度变更调用 `update_plan`；结构调整调用 `replan`。重大 replan 前交互模式先 `ask_user_question`（kind=plan_approval）；目标模式直接 `replan`。
9. **完整成片顺序**：`remaining_plan` 须遵守 canonical：`storyboard_refine_agent` 为剪辑前最后一步；AI 视频须 `video_agent` → `tts_agent` → `storyboard_refine_agent` → `editing_agent`（禁止复核后再生视频）。"""
