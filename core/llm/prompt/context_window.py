...
from core.conversation import ConversationMessage, ConversationStore
from core.llm.agent.react_core import AgentRunContext
from core.llm.model.chat_message import flatten_content_blocks
from core.llm.prompt.chat_messages import conversation_messages_to_chat_blocks
...
def conversation_to_history_lines(messages: list[ConversationMessage]) -> list[str]:
    """将隔离会话消息转为可送入 LLM 的历史行（与 block 映射 flatten 结果一致）。"""
    lines: list[str] = []
    for chat_msg in conversation_messages_to_chat_blocks(messages):
        flat = flatten_content_blocks(chat_msg.get("content", []))
        if flat.strip():
            lines.append(flat)
    return lines
