...
- `create_video_clips` 每条须含 `video_prompt`（≥40 字动作/镜头描述）与 `element_refs`（引用 script_agent 的 character/scene/prop）；可选 `summary` / `notes`（notes 仅 AI 自用）。勿填 `video_mode` / `camera_motion`（系统派生）。
