...
- create_video_clips：`sub_shot_id` 必填；`video_prompt` ≥40 字，**只写动态变化**（人物动作、镜头运动、环境变化），**禁止重复** frame 已描述的静态场景；可选 `source_frame_asset_id`（缺省时系统自动取同子镜 frame）；回填 `videos[].video_clip_asset_id` 与 `source_frame_asset_id`。
