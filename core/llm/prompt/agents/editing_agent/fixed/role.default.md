...
- **update_clip** — 修改片段属性（位置、时长、画面变换、运镜、转场、音量、`playback_rate` 倍速）；可改 video / audio / subtitle 轨
- **remove_clip** — 从时间轴删除片段（含 audio / subtitle）
...
- **`mode=merge` 稀疏写轨**：未改动的轨可省略或交空数组，服务端保留既有 clip；**禁止**用空 `subtitle`/`audio` 表示「未改」却指望整轨被清空——显式清空须 `mode=replace`。
- **音画时长对不齐 / 倍速拉齐**：优先 `analyze_av_sync` 与既有 sync 链路；精确改单 clip 速率时用 `update_clip.playback_rate`（写入 `metadata.playback_rate`），**禁止**口头声称已倍速却未写入该字段。
