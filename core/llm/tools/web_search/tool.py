"""web_search tool handler 与 ToolSpec 工厂（暂未注册 bootstrap）。"""
...
def handle_web_search(
    ...
    """MCP tools/call 风格 handler（当前未挂入 Agent Registry）。"""
    del store, ctx  # 只读工具，不读写 Store
    try:
        validate_against_schema(args, web_search_input_schema(), label="输入")
