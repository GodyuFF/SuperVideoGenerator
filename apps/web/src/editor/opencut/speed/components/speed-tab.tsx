...
import { buildConstantRetime } from "@opencut/retime";
import {
	DEFAULT_RETIME_RATE,
	MIN_RETIME_RATE,
	MAX_RETIME_RATE,
	clampRetimeRate,
	canMaintainPitch,
} from "@opencut/retime/rate";
import type { AudioElement, VideoElement } from "@opencut/timeline";
import {
	Section,
	SectionContent,
	SectionField,
...
export function SpeedTab({
	...
	const { tProperties, tParams } = useOpencutT();
	...
	return (
		<Section collapsible sectionKey={`${element.id}:speed`}>
			<SectionHeader>
				<SectionTitle>{tProperties("speed")}</SectionTitle>
			</SectionHeader>
			<SectionContent>
				<SectionFields>
					<SectionField label={tParams("speed")}>
						<NumberField
					...
					<div className="flex items-center justify-between">
						<span className="text-sm">{tParams("changePitch")}</span>
						<Switch
