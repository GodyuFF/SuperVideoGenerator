import type { MaskableElement } from "@opencut/timeline";
import type { Mask, MaskType, TextMask } from "@opencut/masks/types";
import type { NumberParamDefinition, SelectParamDefinition } from "@opencut/params";
import {
...
import { useMenuPreview } from "@opencut/editor/use-menu-preview";
import { getVisibleElementsWithBounds } from "@opencut/preview/element-bounds";
...
import { ColorPicker } from "@opencut/components/ui/color-picker";
import { FontPicker } from "@opencut/components/ui/font-picker";
import {
	DropdownMenu,
	DropdownMenuContent,
	DropdownMenuItem,
...
import { NumberField } from "@opencut/components/ui/number-field";
import {
	Select,
	SelectContent,
	SelectItem,
...
import { Textarea } from "@opencut/components/ui/textarea";
import {
	Tooltip,
	TooltipContent,
	TooltipTrigger,
...
import { usePropertyDraft } from "@opencut/components/editor/panels/properties/hooks/use-property-draft";
import {
	OcMirrorIcon,
	OcShapesIcon,
	OcTextHeightIcon,
	OcTextWidthIcon,
} from "@opencut/components/icons";
import { cn } from "@opencut/utils/ui";
import { useOpencutT } from "@opencut/i18n/useOpencutT";
import { translateParamOption } from "@opencut/i18n/translateParamLabel";
...
export function MasksTab({ element, trackId }: MasksTabProps) {
	...
	const tracks = useEditor(
		(e) => e.timeline.getPreviewTracks() ?? e.scenes.getActiveSceneOrNull()?.tracks ?? null,
	);
	...
	return (
		<div className="flex flex-col h-full">
			<div className="border-b px-3.5 h-11 shrink-0 flex items-center justify-between gap-2">
				<SectionTitle>{tProperties("masks")}</SectionTitle>
				<DropdownMenu
					...
					{hasMask ? (
						<Tooltip>
							<TooltipTrigger asChild>
								<span className="inline-flex">
									<Button
										...
										size="icon"
										disabled
										aria-label={tProperties("addMask")}
									>
							...
							<TooltipContent className="max-w-56 text-balance">
								{tProperties("oneMaskOnlyTooltip")}
							</TooltipContent>
						</Tooltip>
					) : (
						<DropdownMenuTrigger asChild>
							<Button variant="ghost" size="icon" aria-label={tProperties("addMask")}>
								<HugeiconsIcon icon={PlusSignIcon} className="size-3.5!" />
							</Button>
						</DropdownMenuTrigger>
					)}
				...
				trailing={
					<div className="flex items-center gap-1">
						<Button
							variant="ghost"
							size="icon"
							aria-label={tProperties("toggleMaskInversion", {
						...
						<Button
							variant="ghost"
							size="icon"
							aria-label={tProperties("removeMask", { name: definition.name })}
							onClick={() =>
		...
		<SectionFields>
			...
			{definition.features.hasPosition &&
				...
				"centerY" in mask.params && (
					<SectionField label={tProperties("position")}>
						<div className="flex items-center gap-2">
							<MaskNumberField
								className="flex-1"
								icon="X"
			...
			{definition.features.sizeMode === "width-height" &&
				...
				"height" in mask.params && (
					<SectionField label={tProperties("size")}>
						<div className="flex items-center gap-2">
							<MaskNumberField
								className="flex-1"
								icon="W"
			...
			{definition.features.sizeMode === "height-only" &&
				"height" in mask.params && (
					<SectionField label={tProperties("height")}>
						<MaskNumberField
							icon="H"
							param={getNumberParamDefinition({
								definition,
			...
			{definition.features.sizeMode === "width-only" &&
				"width" in mask.params && (
					<SectionField label={tProperties("width")}>
						<MaskNumberField
							icon="W"
							param={getNumberParamDefinition({
								definition,
			...
			{definition.features.sizeMode === "uniform" && "scale" in mask.params && (
				<SectionField label={tProperties("scale")}>
					<MaskNumberField
						icon={
			...
			{definition.features.hasRotation && "rotation" in mask.params && (
				<SectionField label={tProperties("rotation")}>
					<MaskNumberField
			...
			<SectionField label={tProperties("feather")}>
				<MaskNumberField
			...
			<SectionField label={tProperties("stroke")}>
							...
							<SelectContent>
								{strokeAlignParam.options.map((option) => (
									<SelectItem key={option.value} value={option.value}>
										{translateParamOption("strokeAlign", option.value, option.label)}
									</SelectItem>
								))}
							</SelectContent>
						</Select>
...
function TextMaskFields({
	...
	return (
		<>
			<SectionField label={tProperties("content")}>
				<Textarea
			...
			<SectionField label={tProperties("font")}>
				<FontPicker
			...
			<SectionField label={tProperties("spacing")}>
...
function EmptyView({ onAddMask }: EmptyViewProps) {
	...
	return (
		<div className="flex flex-col h-full items-center justify-center gap-4 text-center">
			...
			<div className="flex flex-col gap-2">
				<h3 className="font-medium text-foreground">{tProperties("noMasks")}</h3>
				<p className="text-muted-foreground text-sm text-balance max-w-40">
					{tProperties("addMaskHint")}
				</p>
			</div>
