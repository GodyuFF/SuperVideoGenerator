...
import { DEFAULT_LOGO_URL, SITE_URL } from "@opencut/site/brand";
...
/** 落地页顶栏：导航、GitHub 与主题切换。 */
export function Header() {
	const { tLanding } = useOpencutT();
	const [isMenuOpen, setIsMenuOpen] = useState(false);
	const closeMenu = () => setIsMenuOpen(false);

	const links = [
		{
			label: tLanding("roadmap"),
			href: "/roadmap",
		},
		{
			label: tLanding("contributors"),
			href: "/contributors",
		},
		{
			label: tLanding("sponsors"),
			href: "/sponsors",
		},
		{
			label: tLanding("blog"),
					...
					<ContextMenu>
						...
						<ContextMenuContent>
							...
							>
								...
								{tLanding("downloadSvg")}
							</ContextMenuItem>
							<Link href="/brand">
								<ContextMenuItem>
									<HugeiconsIcon icon={LinkSquare02Icon} />
									{tLanding("brandAssets")}
								</ContextMenuItem>
							</Link>
						</ContextMenuContent>
					</ContextMenu>
