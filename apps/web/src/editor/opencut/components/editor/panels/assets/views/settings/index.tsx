...
import { FPS_PRESETS } from "@opencut/fps/presets";
import { floatToFrameRate, frameRateToFloat } from "@opencut/fps/utils";
...
import { Tabs, TabsList, TabsTrigger } from "@opencut/components/ui/tabs";
import { Button } from "@opencut/components/ui/button";
import { NumberField } from "@opencut/components/ui/number-field";
import { useEditorStore } from "@opencut/editor/editor-store";
...
import { dimensionToAspectRatio } from "@opencut/utils/geometry";
import { formatNumberForDisplay } from "@opencut/utils/math";
import { OcSquarePlusIcon } from "@opencut/components/icons";
	...
	return (
		<PanelView
			...
			actions={
				...
				>
					<TabsList>
						<TabsTrigger value="project-info">{tAssets("settingsProjectInfo")}</TabsTrigger>
						<TabsTrigger value="background">{tAssets("settingsBackground")}</TabsTrigger>
					</TabsList>
				</Tabs>
			}
		>
			{view === "project-info" && (
				<div className="flex flex-col">
					<Section showTopBorder={false}>
						<SectionHeader>
							<SectionTitle className="flex-1">{tAssets("settingsName")}</SectionTitle>
					...
					<Section showTopBorder={false}>
						<SectionHeader className="justify-between">
							<SectionTitle className="flex-1">{tAssets("settingsFrameRate")}</SectionTitle>
					<Select
							...
							>
								<SelectTrigger className="bg-transparent border-none p-1 h-auto">
									<SelectValue placeholder={tAssets("settingsSelectFrameRate")} />
								</SelectTrigger>
								<SelectContent>
					...
					>
						<SectionHeader>
							<SectionTitle className="flex-1">{tAssets("settingsAspectRatio")}</SectionTitle>
						</SectionHeader>
									...
									uiOptions={
										<div className=" flex items-center gap-2 text-foreground">
											<NumberField
												...
												className="w-full"
												aria-label={tAssets("settingsCanvasWidth")}
											...
											<NumberField
												...
												className="w-full"
												aria-label={tAssets("settingsCanvasHeight")}
