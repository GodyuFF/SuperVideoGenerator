...
export function MediaView() {
	...
	const processFiles = async ({ files }: { files: File[] }) => {
		...
		if (!activeProject) {
			toast.error(tAssets("noActiveProject"));
			return;
		}
...
function MediaItemWithContextMenu({
	...
	const deleteLabel =
		idsToDelete.length > 1
			? tAssets("deleteCountItems", { count: idsToDelete.length })
			: tAssets("delete");
	...
	return (
		<ContextMenu>
			...
			<ContextMenuContent>
				<ContextMenuItem>{tAssets("exportClips")}</ContextMenuItem>
				<ContextMenuItem
					variant="destructive"
...
function MediaPreview({
	...
	return (
		<MediaTypePlaceholder icon={Image02Icon} label={tAssets("typeUnknown")} variant="muted" />
	);
}
...
function MediaActions({
	...
	const sortLabels: Record<MediaSortKey, string> = {
		name: tAssets("sortName"),
		type: tAssets("sortType"),
		duration: tAssets("sortDuration"),
		size: tAssets("sortSize"),
	};
	return (
			...
			<TooltipProvider>
				<Tooltip>
					...
					<TooltipContent>
						<p>
							{mediaViewMode === "grid"
								? tAssets("switchToListView")
								: tAssets("switchToGridView")}
						</p>
					</TooltipContent>
				</Tooltip>
				<Tooltip>
