...
import { SectionField } from "@opencut/components/section";
import { NumberField } from "@opencut/components/ui/number-field";
import { Switch } from "@opencut/components/ui/switch";
...
import {
	translateKeyframeToggleTitle,
	translateParamLabel,
	translateParamOption,
} from "@opencut/i18n/translateParamLabel";
...
export function PropertyParamField({
	...
	const label = translateParamLabel(param.key, param.label);
			...
			beforeLabel={
				keyframe && param.keyframable !== false ? (
					<KeyframeToggle
						...
						title={translateKeyframeToggleTitle(label)}
				...
				<SelectContent>
					{param.options.map((option) => (
						<SelectItem key={option.value} value={option.value}>
							{translateParamOption(param.key, option.value, option.label)}
						</SelectItem>
					))}
				</SelectContent>
			</Select>
