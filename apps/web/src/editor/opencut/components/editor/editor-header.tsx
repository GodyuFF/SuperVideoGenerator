...
import { RenameProjectDialog } from "@opencut/project/components/rename-project-dialog";
import { DeleteProjectDialog } from "@opencut/project/components/delete-project-dialog";
import { useRouter } from "next/navigation";
import { FaDiscord } from "react-icons/fa6";
import { ExportButton } from "./export-button";
import { FeedbackPopover } from "@opencut/feedback/components/feedback-popover";
import { ThemeToggle } from "../theme-toggle";
import { DEFAULT_LOGO_URL } from "@opencut/site/brand";
	...
	const handleSaveProjectName = async (newName: string) => {
		if (
			...
			try {
				...
				toast.error(tDialogs("failedRenameProject"), {
					description:
						error instanceof Error ? error.message : tDialogs("pleaseTryAgain"),
				});
			} finally {
				setOpenDialog(null);
			}
	...
	const handleDeleteProject = async () => {
		if (activeProject) {
			try {
				...
				toast.error(tDialogs("failedDeleteProject"), {
					description:
						error instanceof Error ? error.message : tDialogs("pleaseTryAgain"),
				});
			} finally {
				...
				<DropdownMenuTrigger asChild>
					<Button variant="ghost" size="icon" className="p-1 rounded-sm size-8">
						<Image
							...
							alt={tDialogs("projectThumbnailAlt")}
							width={32}
							height={32}
