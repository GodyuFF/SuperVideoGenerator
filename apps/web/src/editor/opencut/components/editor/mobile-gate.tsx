...
export function MobileGate({ children }: MobileGateProps) {
	const { tDialogs, tLanding } = useOpencutT();
	...
	return (
		<div className="bg-background relative flex h-screen w-screen flex-col overflow-hidden">
			...
			>
				...
				<span className=" text-sm">{tDialogs("mobileGateGoBack")}</span>
			</Button>

			<div className="flex flex-1 flex-col justify-center gap-5 px-7">
				<div className="flex flex-col gap-3">
					<h1 className="text-foreground text-3xl font-bold tracking-tight">
						{tDialogs("mobileGateTitleDetailed")}
					</h1>
					<p className="text-muted-foreground text-sm leading-relaxed">
						{tDialogs("mobileGateBodyDetailed")}
					</p>
				</div>
				<div className="flex items-center gap-3">
					<Button onClick={handleContinue}>{tDialogs("mobileTakeLookAnyway")}</Button>
