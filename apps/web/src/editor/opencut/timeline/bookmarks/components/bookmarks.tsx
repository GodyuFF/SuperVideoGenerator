...
import { DEFAULT_TIMELINE_BOOKMARK_COLOR } from "@opencut/timeline/components/theme";
import { TIMELINE_BOOKMARK_ROW_HEIGHT_PX } from "@opencut/timeline/components/layout";
...
import { clamp, formatNumberForDisplay } from "@opencut/utils/math";
import { timelineTimeToPixels, timelineTimeToSnappedPixels } from "@opencut/timeline";
import {
	type MediaTime,
...
export function TimelineBookmarksRow({
	...
	const bookmarks = useEditor((e) => e.scenes.getActiveSceneOrNull()?.bookmarks ?? []);
	...
	return (
		...
		>
			<button
				...
				aria-label={tTimeline("ariaTimelineRuler")}
				type="button"
				onWheel={handleWheel}
				onClick={(event) => {
		...
		<Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
			<PopoverAnchor asChild>
				<button
					...
					aria-label={tTimeline("bookmarkAt", {
	...
	return (
		<>
			<div className="flex flex-col gap-2">
				<Label className="text-xs">{tTimeline("note")}</Label>
				<Input
					placeholder={tTimeline("addNote")}
					value={bookmark.note ?? ""}
			...
			<div className="flex flex-col gap-2">
				<Label className="text-xs">{tTimeline("color")}</Label>
					...
					{bookmark.color &&
						bookmark.color.replace(/^#/, "").toUpperCase() !==
							...
							<Button
								...
								variant="text"
								size="text"
								aria-label={tTimeline("resetColor")}
			...
			<div className="flex flex-col gap-2">
				<Label className="text-xs">{tTimeline("duration")}</Label>
			...
			<Button
				...
				aria-label={tTimeline("deleteBookmark")}
			>
