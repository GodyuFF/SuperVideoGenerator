...
import type { Effect } from "@opencut/effects/types";
import type { EffectElement, VisualElement } from "@opencut/timeline";
import { effectsRegistry } from "@opencut/effects";
	...
	return (
		<div className="flex flex-col h-full">
			<div className="border-b px-3.5 h-11 shrink-0 flex items-center">
				<SectionTitle>{tProperties("effect")}</SectionTitle>
			</div>
			<EffectSection
				effect={effect}
...
export function ClipEffectsTab({
	...
	return (
		<div className="flex flex-col h-full">
			<div className="border-b px-3.5 h-11 shrink-0 flex items-center">
				<SectionTitle>{tProperties("effects")}</SectionTitle>
			</div>
			{effects.length === 0 ? (
				<EmptyView />
			) : (
	...
	return (
		<div className="flex flex-col h-full items-center justify-center gap-4 text-center">
			...
			<div className="flex flex-col gap-2">
				<h3 className="font-medium text-foreground">{tProperties("noEffects")}</h3>
				<p className="text-muted-foreground text-sm text-balance max-w-44">
					{tProperties("effectsFromAssets")}
				</p>
			</div>
			<Button
				variant="default"
					...
					onToggle && (
						<div className="flex items-center gap-1">
							<Button
								...
								size="icon"
								aria-label={tProperties("toggleEffect", {
							...
							<Button
								variant="ghost"
								size="icon"
								aria-label={tProperties("removeEffect", {
									effect: definition.name,
								})}
								onClick={onRemove}
							>
