...
import { extractTimelineAudio } from "@opencut/media/mediabunny";
import { useEditor } from "@opencut/editor/use-editor";
import { TRANSCRIPTION_DIAGNOSTICS_SCOPE } from "@opencut/transcription/diagnostics";
import { DEFAULT_TRANSCRIPTION_SAMPLE_RATE } from "@opencut/transcription/audio";
import { TRANSCRIPTION_LANGUAGES } from "@opencut/transcription/supported-languages";
import type {
	CaptionChunk,
	TranscriptionLanguage,
	TranscriptionProgress,
} from "@opencut/transcription/types";
import { transcriptionService } from "@opencut/services/transcription/service";
import { decodeAudioToFloat32 } from "@opencut/media/audio";
import { buildCaptionChunks } from "@opencut/transcription/caption";
...
import type { DiagnosticSeverity } from "@opencut/diagnostics/types";
...
export function Captions() {
	...
	const handleProgress = (progress: TranscriptionProgress) => {
		if (progress.status === "loading-model") {
			dispatch({
				type: "update_step",
				step: tAssets("loadingModel", {
					progress: Math.round(progress.progress),
				}),
			});
		} else if (progress.status === "transcribing") {
			dispatch({ type: "update_step", step: tAssets("transcribing") });
		}
	};
	...
	const handleGenerateTranscript = async () => {
		dispatch({ type: "start", step: tAssets("extractingAudio") });
		try {
			...
			dispatch({ type: "update_step", step: tAssets("preparingAudio") });
			...
			dispatch({ type: "update_step", step: tAssets("generatingCaptions") });
	...
	const handleImportFile = async ({ file }: { file: File }) => {
		dispatch({ type: "start", step: tAssets("readingSubtitleFile") });
		try {
			...
			dispatch({ type: "update_step", step: tAssets("importingSubtitles") });
		...
		>
			...
			>
				<SectionContent className="flex flex-col gap-4 h-full pt-1">
					<SectionFields>
						<SectionField label={tAssets("language")}>
							<Select
								value={selectedLanguage}
								onValueChange={(value) => handleLanguageChange({ value })}
							>
								<SelectTrigger>
									<SelectValue placeholder={tAssets("selectLanguage")} />
								</SelectTrigger>
								<SelectContent>
									<SelectItem value="auto">{tAssets("autoDetect")}</SelectItem>
					...
					>
						...
						{isProcessing ? processing.step : tAssets("generateTranscript")}
					</Button>
					{error && (
