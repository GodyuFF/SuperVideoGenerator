import type { SceneTracks, TimelineTrack } from "@opencut/timeline";
...
import type { TBackground, TCanvasSize } from "@opencut/project/types";
import { DEFAULT_BACKGROUND_BLUR_INTENSITY } from "@opencut/background/blur";
import {
	buildTransformFromParams,
	readBlendModeFromParams,
	readOpacityFromParams,
} from "@opencut/rendering";
	...
	for (const track of tracks) {
		...
		for (const element of elements) {
			...
			if (element.type === "video" || element.type === "image") {
				...
				if (!mediaAsset?.file || !mediaAsset?.url || mediaAsset.file.size === 0) {
					continue;
				}
