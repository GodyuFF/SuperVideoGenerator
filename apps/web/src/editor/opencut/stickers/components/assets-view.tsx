...
import { resolveStickerIntrinsicSize } from "@opencut/stickers";
import {
	buildGraphicElement,
	buildStickerElement,
} from "@opencut/timeline/element-utils";
import { STICKER_CATEGORIES } from "@opencut/stickers/categories";
import { getRegionLabel, resolveQueryToRegions } from "@opencut/stickers";
import { parseShapeStickerId } from "@opencut/stickers/providers/shapes";
import type { TimelineDragData } from "@opencut/timeline/drag";
import type {
	StickerBrowseSection,
	StickerCategory,
	StickerItem as StickerData,
} from "@opencut/stickers";
import { useStickersStore } from "@opencut/stickers/stickers-store";
		...
		<div className="flex h-full flex-col py-2">
			<div className="px-2">
				<Input
					size="sm"
					variant="default"
					placeholder={tAssets("searchPlaceholder")}
					value={searchQuery}
					onChange={(e) => {
			...
			>
				<TabsList aria-label={tAssets("stickerCategories")}>
	...
	return (
		<div className="bg-background flex h-full flex-col items-center justify-center gap-3 p-4">
			...
			<div className="flex flex-col gap-2 text-center">
				<p className="text-lg font-medium">{tAssets("noStickersFound")}</p>
...
function RegionBanner({ region }: { region: string }) {
	return (
		<div className="flex h-7 items-center gap-1.5 rounded-lg border border-border bg-secondary px-2 text-muted-foreground">
			<svg
				width="12"
				height="12"
				viewBox="0 0 24 24"
...
function StickersContentView() {
	...
	if (viewMode === "search") {
				...
				<div className="flex flex-col gap-3 pb-4">
					...
					<div className="flex items-center justify-between">
						<span className="text-muted-foreground text-sm">
							{tAssets("resultsCount", { count: searchResults.total })}
						</span>
					</div>
		...
		if (selectedCategory !== "all" && searchQuery) {
			return <EmptyView message={tAssets("noStickersForQuery", { query: searchQuery })} />;
		}
	}
	...
	if (!browseContent?.sections.length) {
		...
		return (
			<EmptyView
				message={
					viewMode === "search"
						? tAssets("noStickersForQuery", { query: searchQuery })
						: selectedCategory === "all"
							? tAssets("noStickersAvailable")
							: tAssets("noStickersInCategory", {
...
function StickerItem({
	...
	const handleAdd = async () => {
		...
		try {
			...
			toast.error(tAssets("failedAddSticker"));
		} finally {
			setIsAdding(false);
		}
	};
