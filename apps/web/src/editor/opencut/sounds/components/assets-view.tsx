...
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@opencut/components/ui/tabs";
import { useInfiniteScroll } from "@opencut/hooks/use-infinite-scroll";
import { useSoundSearch } from "@opencut/sounds/use-sound-search";
import { useSoundsStore } from "@opencut/sounds/sounds-store";
import type { SavedSound, SoundEffect } from "@opencut/sounds/types";
		...
		<div className="flex h-full flex-col">
			<Tabs defaultValue="sound-effects" className="flex h-full flex-col">
				<div className="px-3 pt-4 pb-0">
					<TabsList>
						<TabsTrigger value="sound-effects">{tAssets("soundEffects")}</TabsTrigger>
						<TabsTrigger value="saved">{tAssets("saved")}</TabsTrigger>
					</TabsList>
				</div>
	...
	return (
		<div className="mt-1 flex h-full flex-col gap-5">
			<div className="flex items-center gap-3">
				<Input
					placeholder={tAssets("searchSounds")}
					className="w-full"
				...
				<DropdownMenu>
					...
					<DropdownMenuContent align="end" className="w-56">
						...
						>
							{tAssets("commercialOnly")}
						</DropdownMenuCheckboxItem>
						<div className="text-muted-foreground px-2 py-1.5 text-xs">
							{showCommercialOnly
								? tAssets("commercialOnlyHint")
								: tAssets("allSoundsHint")}
						</div>
					</DropdownMenuContent>
				</DropdownMenu>
			</div>
				...
				>
					<div className="flex flex-col gap-4">
						...
						{!isLoading && !isSearching && displayedSounds.length === 0 && (
							<div className="text-muted-foreground text-sm">
								{searchQuery
									? tAssets("noSoundsFound")
									: tAssets("noSoundsAvailable")}
							</div>
						)}
						{isLoadingMore && (
							<div className="text-muted-foreground py-4 text-center text-sm">
								{tAssets("loadingMoreSounds")}
							</div>
						)}
					</div>
				</ScrollArea>
		...
		return (
			<div className="flex h-full items-center justify-center">
				<div className="text-destructive text-sm">
					{tAssets("errorPrefix", { message: savedSoundsError })}
				</div>
			</div>
		);
	}
		...
		return (
			<div className="bg-background flex h-full flex-col items-center justify-center gap-3 p-4">
				...
				<div className="flex flex-col gap-2 text-center">
					<p className="text-lg font-medium">{tAssets("noSavedSounds")}</p>
					<p className="text-muted-foreground text-sm text-balance">
						{tAssets("saveSoundHint")}
					</p>
				</div>
			</div>
		);
		...
		<div className="mt-1 flex h-full flex-col gap-5">
			<div className="flex items-center justify-between">
				<p className="text-muted-foreground text-sm">
					{tAssets("savedCount", { count: savedSounds.length })}
				</p>
				<Dialog open={showClearDialog} onOpenChange={setShowClearDialog}>
					...
					<DialogContent>
						<DialogHeader>
							<DialogTitle>{tAssets("clearAllSavedTitle")}</DialogTitle>
							<DialogDescription>
								{tAssets("clearAllSavedDesc", { count: savedSounds.length })}
							</DialogDescription>
						</DialogHeader>
						<DialogFooter>
		...
		<div className="group flex items-center gap-3 opacity-100 hover:opacity-75">
			...
			<div className="flex items-center gap-3 pr-2">
				...
				<Button
					...
					title={isSaved ? tAssets("removeFromSaved") : tAssets("saveSound")}
				>
					<HugeiconsIcon
						icon={FavouriteIcon}
