...
> 日期：2026-07-17  
> 状态：草案（待实现）  
> 相关：`docs/orchestration-state.md`、`core/llm/master/pipeline_progress.py`、`core/llm/master/master_react.py`
...
| `docs/orchestration-state.md` | 文档同步：seed 增加 LLM 路径与 `reopen_intent` 字段 |
| `docs/prompt-architecture.md` | 动态槽位表增加 `reopen_intent` |
...
- `docs/orchestration-state.md` §4.2 Seed、字段一览  
- `docs/prompt-architecture.md` 动态槽位（`reopen_intent`）  
