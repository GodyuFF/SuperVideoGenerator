...
执行期间左侧输入框与剧本手动编辑会暂时锁定（界面提示「AI 执行中…」），属预期行为；完成后自动恢复。若切换剧本后仍无法输入，点一次 **「中止执行」** 或刷新页面即可。
...
- 中途想停：用确认卡片 **「中止」** 或顶栏相关中止控件（若正在执行）。中止后应立刻可再发消息；若按钮仍停在「中止」，刷新页面即可。
- 风格与模式对照见 [视频风格与模式](06-modes.md)。
- **Skill**：输入 `/` 选择（如 `/short_drama`）。可选列表受当前风格对应 Profile 下「超级视频大师」的 Skill 白名单限制（在 **Agent 配置** 中按与工具相同的「添加 / 移除」管理）。**导入 / 删除** 在顶栏 **Skill 库**（`#/skills`），导入前会校验 zip 格式。激活后会**跨轮保持**；长参考由 Agent 按需 `read_skill_ref` 拉取。
